...
Component({
  ...
  attached: function () {
    ...
    curPage.onPageScroll = (rest) => {
        ...
        // 自己的逻辑
        if (rest.scrollTop > that.data.navbarHeight) {
          that.setData({status: 'center'})
        } else {
          that.setData({status: 'left'})
        }
    };
    ...
    console.log( app.globalData.systeminfo.windowWidth, headerPosi.width, 'JJJJJJJJJJ')
    this.setData({
  ...
  methods: {
    onPageScroll (e) {
      console.log(e, 'OOOOOOOOOOOOO', this)
    },
    _goBack: function () {
      wx.navigateBack({
        delta: 1
